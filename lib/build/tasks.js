...
const timeUtils = require(__dirname + '/../converter/timeutils.js')
	...
	, { Assets, CustomPages, Stats, Posts, Files, Boards, News, Modlogs } = require(__dirname + '/../../db/')
...
module.exports = {
	...
	buildCustomPages: async (options) => {
		const label = '/pages.html';
		const start = process.hrtime();
		const { html, json } = await render(label, 'pages.pug', options, {
			'name': '/pages.json',
			'data': options.custompages ? options.custompages : ''
		});
		const end = process.hrtime(start);
		debugLogs && console.log(timeDiffString(label, end));
		return { html, json };
	...
	buildLogos: async (options) => {
		const label = '/logos.html';
		const start = process.hrtime();
		const { html, json } = await render(label, 'logos.pug', options, {
			'name': '/logos.json',
			'data': options.logos ? options.logos : ''
		});
		const end = process.hrtime(start);
		debugLogs && console.log(timeDiffString(label, end));
		return { html, json };
	...
	buildBanners: async (options) => {
		const label = '/banners.html';
		const start = process.hrtime();
		const { html, json } = await render(label, 'banners.pug', options, {
			'name': '/banners.json',
			'data': options.banners ? options.banners : ''
		});
		const end = process.hrtime(start);
		debugLogs && console.log(timeDiffString(label, end));
		return { html, json };
	...
	buildCatalog: async (options) => {
		...
		options.board.flags = (await Assets.getFlags()).
			map(filename => {
	...
	buildThread: async (options) => {
		...
		let htmlFull, jsonFull;
		({ html: htmlFull, json: jsonFull } = await render(`/${options.board._id || options.board}/thread/${options.threadId}.html`, 'thread.pug', {
			...options,
			thread,
		}, {
		...
		let htmlLast, jsonLast;
		if (thread.replyposts && thread.replyposts > 50) {
			options.managePage = `thread/${options.threadId}+50.html`;
			thread.replies = thread.replies.slice(-50);
			({ html: htmlLast, json: jsonLast } = await render(`/${options.board._id || options.board}/thread/${options.threadId}+50.html`, 'thread.pug', {
				...options,
				thread,
				lastReplies: true,
			}, {
				'name': `/${options.board._id}/thread/${options.threadId}+50.json`,
				'data': thread
			}));
		}
		...
		if (options.lastReplies && htmlLast && jsonLast) {
			return { html: htmlLast, json: jsonLast };
		} else {
			return { html: htmlFull, json: jsonFull };
		}
	},

	buildBoard: async (options) => {
		const label = `/${options.board._id || options.board}/${options.page === 1 ? 'index' : options.page}.html`;
		const start = process.hrtime();
		if (!options.board._id) {
			options.board = await Boards.findOne(options.board);
		}
	...
	buildCustomPage: async (options) => {
		const label = `/custompage/${options.page}.html`;
		const start = process.hrtime();
		if (!options.customPage) {
			const customPage = await CustomPages.findOne(options.page);
			if (!customPage) {
				return {};
			}
		...
		const { html, json } = await render(label, 'custompage.pug', {
			...
			name: `/custompage/${options.page}.json`,
