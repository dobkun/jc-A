...
const Posts = require(__dirname + '/../../db/posts.js')
	...
	, quoteRegexLiteral = />>(?<quotenum>\d+)/g
...
module.exports = {
	...
	replace: async (text, sourceBoard, map) => {
		...
		const quotes = text.match(quoteRegexLiteral);
		if (!quotes) {
			return text;
		}
		...
		for (let i = 0; i < quotes.length; i++) {
			const quote = quotes[i].substring(2);
			...
			if (map[quote]) {
				...
				// quote to source board, outside moved thread
				newQuote = `>>>/${sourceBoard}/${quote}`;
			}
			console.log('quote', quote, 'newQuote', newQuote);
			text = text.replace(new RegExp(`>>${quote}`, 'g'), newQuote);
		}
